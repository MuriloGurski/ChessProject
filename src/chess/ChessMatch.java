...
public class ChessMatch {
    ...
    public Piece makeMove(Position sourcePos, Position targetPos){
        ...
        if (p instanceof Pawn){
            if(sourcePos.getCollumn() != targetPos.getCollumn() && capturedPiece == null){
                Position pawnPosition;
                if (p.getColor() == Color.WHITE){
                    pawnPosition = new Position(targetPos.getRow()+1, targetPos.getCollumn());
                }
                else {
                    pawnPosition = new Position(targetPos.getRow()-1, targetPos.getCollumn());
                }
    ...
    private void undoMove(Position source, Position target, Piece captured){
        ...
        //enpassant
        if (p instanceof Pawn){
            if(source.getCollumn() != target.getCollumn() && captured == enPassantVulnerable){
